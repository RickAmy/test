...
#     '''
#     # 从环境变量中获取支持的文件扩展名
#     supported_extensions = os.getenv('SUPPORTED_EXTENSIONS', '.c,.rs').split(',')

#     filtered_changes = []
#     for item in changes:
#         # 兼容 Gerrit (filename) 和 GitLab (new_path) 的字段名
#         file_path = item.get('filename') or item.get('new_path', '')
#         #logger.info(f"Checking file: {file_path}")#打印文件路径
#         if not file_path:
#             continue
...
#         #         'diff': item.get('diff', ''),
#         #         'filename': file_path,  # 统一字段名
#         #         'additions': item.get('insertions', 0),  # Gerrit 用 insertions/deletions
#         #         'deletions': item.get('deletions', 0)
#         #     })
#         # 检查文件扩展名
#         if any(file_path.endswith(ext) for ext in supported_extensions):
#             # 处理新增文件的情况
#             if item.get('status') == 'added' or item.get('new_file'):
#                 # 对于新增文件，我们可以设置一个空的diff或者获取文件内容
#                 diff_content = item.get('diff', '') or f"# New file: {file_path}"
#             else:
...
#                 'diff': diff_content,
#                 'filename': file_path,
#                 'additions': item.get('insertions', item.get('additions', 0)),
#                 'deletions': item.get('deletions', 0)
#             })
...
def filter_changes(changes: list):
    ...
    #############################
    # 打印原始文件列表
    # original_files = [item.get('filename') or item.get('new_path', '') for item in changes]
    # logger.info(f"过滤前的文件列表: {original_files}")
    ##########################
    ...
    for item in changes:
        ...
        if any(file_path.endswith(ext) for ext in supported_extensions):
            ...
            filtered_changes.append({
                ...
                'additions': item.get('insertions', item.get('additions', 0)),
                'deletions': item.get('deletions', 0),
                'status': item.get('status', 'modified')  # 保留文件状态
            })

    ##########################        
    # 打印过滤后的文件列表
    filtered_files = [item['filename'] for item in filtered_changes]
    logger.info(f"过滤后的文件列表（支持的类型）: {filtered_files}")
    ###############
    return filtered_changes
