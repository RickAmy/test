...
from biz.queue.worker import handle_merge_request_event, handle_push_event, handle_github_pull_request_event, \
    handle_github_push_event, handle_gerrit_event
...
def handle_webhook():
    ...
    if request.is_json:
        ...
        if request.headers.get('X-GitHub-Event'):  # GitHub
            return handle_github_webhook(request.headers['X-GitHub-Event'], data)
        elif request.headers.get('X-Gitlab-Event'):  # GitLab
            return handle_gitlab_webhook(data)
        else:  # 默认为 Gerrit
            return handle_gerrit_webhook(data)
    else:
        return jsonify({'message': 'Invalid data format'}), 400


#def handle_gitlab_webhook(data):
def handle_gerrit_webhook(data):
    try:

        print("=== 收到Gerrit Webhook ===")
        print(json.dumps(data, indent=2))  # 打印原始数据
        ...
        # 生成 Gerrit 的 url_slug（示例逻辑，根据实际需求调整）
        gerrit_url = os.getenv('GERRIT_URL', 'http://10.42.41.50:29418')
        url_slug = gerrit_url.replace('http://', '').replace('https://', '').replace('/', '_')

        # 异步处理（传递所有必要参数）
        handle_queue(
            handle_gerrit_event,
            data,
            os.getenv('GERRIT_ACCESS_TOKEN'),
            gerrit_url,
            url_slug  # 新增参数
        )
        return jsonify({"message": "Gerrit event accepted"}), 200

    except Exception as e:
        logger.error(f"Gerrit webhook error: {e}")
