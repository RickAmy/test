...
def get_data(service_func, authors=None, project_names=None, updated_at_gte=None, updated_at_lte=None, columns=None):
    ...
    if 'review_result' in df.columns:
        # 从review_result文本中提取分数
        df['parsed_score'] = df['review_result'].str.extract(r'总分[:：]\s*(\d+)分?').astype(float)
        # 优先使用解析出的分数
        df['score'] = df['parsed_score'].fillna(df['score'])
...
def main_page():
    ...
    # 在页面顶部添加Gerrit数据开关
    show_gerrit_data = st.checkbox("显示Gerrit数据", value=True)
    
    # 日期选择器
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        start_date = st.date_input("开始日期", start_date_default)
    with col2:
        end_date = st.date_input("结束日期", current_date)
    
    start_datetime = datetime.datetime.combine(start_date, datetime.time.min)
    end_datetime = datetime.datetime.combine(end_date, datetime.time.max)

    # 获取初始数据用于确定可选作者和项目
    initial_data = ReviewService().get_mr_review_logs(
        updated_at_gte=int(start_datetime.timestamp()),
        updated_at_lte=int(end_datetime.timestamp())
    )
    
    if show_gerrit_data:
        gerrit_data = ReviewService().get_gerrit_review_logs(
            updated_at_gte=int(start_datetime.timestamp()),
            updated_at_lte=int(end_datetime.timestamp())
        )
        initial_data = pd.concat([initial_data, gerrit_data])

    unique_authors = sorted(initial_data["author"].dropna().unique().tolist()) if not initial_data.empty else []
    unique_projects = sorted(initial_data["project_name"].dropna().unique().tolist()) if not initial_data.empty else []
    
    with col3:
        authors = st.multiselect("开发者", unique_authors, default=[])
    with col4:
        project_names = st.multiselect("项目名称", unique_projects, default=[])

    # 根据环境变量决定是否显示 push_tab
    #show_push_tab = os.environ.get('PUSH_REVIEW_ENABLED', '0') == '1'
    ######只显示一个默认选项，取消合并请求和Gerrit Patchset选项
    #if show_push_tab:
    #    mr_tab, push_tab = st.tabs(["合并请求", "Gerrit Patchset"])
    #else:
    mr_tab = st.container()
    ...
    def display_data(tab, service_func, columns, column_config):
        with tab:
            ...
            average_score = filtered_data["score"].mean() if not filtered_data.empty else 0
            ...
            with row1:
                st.markdown("<div style='text-align: center; font-size: 20px;'><b>项目提交统计</b></div>",
                            unsafe_allow_html=True)
                generate_project_count_chart(filtered_data)
            with row2:
                st.markdown("<div style='text-align: center; font-size: 20px;'><b>项目平均得分</b></div>",
                            unsafe_allow_html=True)
                generate_project_score_chart(filtered_data)
            with row3:
                st.markdown("<div style='text-align: center; font-size: 20px;'><b>开发者提交统计</b></div>",
                            unsafe_allow_html=True)
                generate_author_count_chart(filtered_data)
            with row4:
                st.markdown("<div style='text-align: center; font-size: 20px;'><b>开发者平均得分</b></div>",
                            unsafe_allow_html=True)
                generate_author_score_chart(filtered_data)

            # 代码行数图表
            if 'additions' in filtered_data.columns and 'deletions' in filtered_data.columns:
                row5 = st.columns(1)[0]
                with row5:
                    st.markdown("<div style='text-align: center;'><b>人员代码变更行数</b></div>", unsafe_allow_html=True)
                    generate_author_code_line_chart(filtered_data)

    # 合并请求数据配置
    mr_columns = ["project_name", "author", "source_branch",  "updated_at", 
                  "commit_messages", "delta", "score", "url", 'additions', 'deletions']
    ...
    mr_column_config = {
        ...
        "url": st.column_config.LinkColumn("操作", display_text="查看详情"),
        "additions": None,
        "deletions": None,
    }

    # 根据是否显示Gerrit数据选择不同的数据源
    if show_gerrit_data:
        # 合并MR和Gerrit数据
        def get_combined_data(**kwargs):
            mr_data = ReviewService().get_mr_review_logs(**kwargs)
            gerrit_data = ReviewService().get_gerrit_review_logs(**kwargs)
            return pd.concat([mr_data, gerrit_data])
            #return mr_data 
        
        display_data(mr_tab, get_combined_data, mr_columns, mr_column_config)
    else:
        display_data(mr_tab, ReviewService().get_mr_review_logs, mr_columns, mr_column_config)

    # # Push数据展示
    # if show_push_tab:
    #     push_columns = ["project_name", "author", "branch", "updated_at", 
    ...
    #         "updated_at": "更新时间",
    #         "commit_messages": "提交信息",
    #         "score": st.column_config.ProgressColumn("得分", min_value=0, max_value=100),
    #         "additions": None,
    #         "deletions": None,
    #     }
